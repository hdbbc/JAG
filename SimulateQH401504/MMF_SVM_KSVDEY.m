...
%%高质量信号与噪声信号
   ...
   figure(4)
     ...
     fids1 = fopen('QH401504TS5EYMMFSVMGood.dat', 'wt');%保存KSVD处理后的噪声的信号
     fprintf(fids1, '%10.0f%10.0f',b);
     fclose(fids1);
     CLzong=load('QH401504TS5EYMMFSVMGood.dat');
     
     fids2 = fopen('QH401504TS5EYMMFSVMNoisy.dat', 'wt');%保存KSVD处理后的噪声的信号
     fprintf(fids2, '%10.0f%10.0f',a);
     fclose(fids2);
     CLzong=load('QH401504TS5EYMMFSVMNoisy.dat');
%% KSVD字典学习去噪
    N=length(a);
    for Step=0:00;
      Dataspan=1:N;
...
%% 保存数据
    ...
    MMFSVMKSVD=MT;
    fids4=fopen('QH401504TS5EYMMFSVMKSVD.dat', 'wt');%保存SVM-KSVD处理后的信号
    fprintf(fids4,'%10.0f%10.0f',MMFSVMKSVD);
    fclose(fids4);
    CLzong=load('QH401504TS5EYMMFSVMKSVD.dat');
 figure
