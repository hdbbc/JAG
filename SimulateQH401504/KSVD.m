...
%                                 error.
%表示任意一个信号所用原子的个数即稀疏度，等于1表示可以用每个原子，那么这时训练误差param.errorGoal就必须输入的。
...
%                                iteration).
%如果真实的字典库有输入的话，就是TrueDictionary有输入，ratio就是每次迭代的系数。
%    totalerr                    The total representation error after each
%                                iteration (defined only if
%                                param.displayProgress=1 and
%                                param.errorFlag = 0)
% 总体误差，（只有当param.displayProgress=1 and param.errorFlag = 0时才有输出）
...
%normalize the dictionary.将字典中的每个原子都归一化
...
for iterNum = 1:param.numIteration
    ...
    %如果目标误差是0的话，则执行OMP，如果目标误差限定了，则执行 OMPerr
    ...
    for j = rPerm
        [betterDictionaryElement,CoefMatrix,addedNewVector] = I_findBetterDictionaryElement(Data,...
            ...
            CoefMatrix ,param.L);
        Dictionary(:,j) = betterDictionaryElement;% 将训练好的原子代替原来的原子
        if (param.preserveDCAtom)%%仅当需要固定某个原子时使用
...
relevantDataIndices = find(CoefMatrix(j,:)); % the data indices that uses the j'th dictionary element.% 这里更新第j个原子，仅选择使用了该原子的信号（为了保持稀疏度）
