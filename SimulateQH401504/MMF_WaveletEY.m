...
%% 选取一段进行小波去噪
 ...
 %% 采用硬阈值的matlab去噪方法
  ...
  figure
    ...
    %% 保存数据
    fids1 = fopen('QH401504TS5EYMMFWaveletRuan.dat', 'wt');%保存KSVD处理后的噪声的信号
    fprintf(fids1, '%10.0f%10.0f',MMFWaveletRuan);
    fclose(fids1);
    CLzong=load('QH401504TS5EYMMFWaveletRuan.dat');
    fids2 = fopen('QH401504TS5EYMMFWaveletYing.dat', 'wt');%保存KSVD处理后的噪声的信号
    fprintf(fids2, '%10.0f%10.0f',MMFWaveletYing);
    fclose(fids2);
    CLzong=load('QH401504TS5EYMMFWaveletYing.dat');
  figure
