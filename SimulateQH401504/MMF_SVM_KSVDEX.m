 ...
 close all;
 clc;
    MTOriginal=load('QH401804TS5HX.dat');
...
%% 形态滤波去噪
     MT_MMF=fcocmk_zhu(RawMT,20,0);%选择合适的结构元素大小
   ...
   figure(3)
     for i=1:k
       ...
       if(datao(i,3:3)==1)%% 强干扰
          plot((N*(i-1)+1:N*i),P(i,:),'k');axis([0 Length -max(x) max(x)]);
          set(gca,'FontName','Times New Roman','FontSize',10);xlabel('Time(s)');ylabel('Amplitude');
       else
   ...
   figure(4)
     ...
     for i=1:k
       ...
       if(datao(i,3:3)==1) 
         ...
         b(N*(i-1)+1:N*i)=P(i,:);
         subplot(313)
         plot((N*(i-1)+1:N*i),P(i,:),'r');axis([ 0 length(x) -max(x) max(x)]);legend('SVM-Good');
         set(gca,'FontName','Times New Roman','FontSize',10);xlabel('Time(s)');ylabel('Amplitude');
         hold on
       end
     end
%      fids1 = fopen('QH401804TS5HXMMFSVMGood.dat', 'wt');%保存KSVD处理后的噪声的信号
%      fprintf(fids1, '%10.0f%10.0f',b);
%      fclose(fids1);
%      CLzong=load('QH401804TS5HXMMFSVMGood.dat');
%      
%      fids2 = fopen('QH401804TS5HXMMFSVMNoisy.dat', 'wt');%保存KSVD处理后的噪声的信号
%      fprintf(fids2, '%10.0f%10.0f',a);
%      fclose(fids2);
%      CLzong=load('QH401804TS5HXMMFSVMNoisy.dat');
%% KSVD字典学习去噪
    N=length(a);
    for Step=0:00;
      Dataspan=1:N;
...
%% KSVD电磁去噪
      ...
      a=OMPerr(Dictionary,Data ,50000); %2 %4 %3
      y=Dictionary*a;
...
%% 保存数据
     MMFSVMKSVD=MT_MMF+Mtdata+b;
%     MT=load('QH401804TS5HX.dat');
%     MT(300001:750000)=MMFSVMKSVD;
%     MMFSVMKSVD=MT;
%     fids4=fopen('QH401804TS5HXMMFSVMKSVD.dat', 'wt');%保存SVM-KSVD处理后的信号
%     fprintf(fids4,'%10.0f%10.0f',MMFSVMKSVD);
%     fclose(fids4);
%     CLzong=load('QH401804TS5HXMMFSVMKSVD.dat');
 figure
